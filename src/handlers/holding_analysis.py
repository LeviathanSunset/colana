...
# 导入OKX相关功能
try:
    from ..services.okx_crawler import OKXCrawlerForBot, format_tokens_table, format_token_holders_detail, analyze_address_clusters, format_cluster_analysis
except ImportError:
...
class HoldingAnalysisHandler:
        ...
        try:
            ...
            if result and result.get('token_statistics'):
                ...
                if table_markup:
                    # 添加排序切换按钮
                    table_markup.add(
                        InlineKeyboardButton("💰 按价值排序", callback_data=f"ca1_sort_value_{cache_key}"),
                        InlineKeyboardButton("👥 按人数排序 ✅", callback_data=f"ca1_sort_count_{cache_key}")
                    )
                    # 添加集群分析按钮
                    table_markup.add(
    ...
    def _handle_sort_callback(self, call: CallbackQuery, sort_by: str, cache_key: str):
        ...
        try:
            ...
            self.bot.answer_callback_query(call.id, f"已切换到{'价值' if sort_by == 'value' else '人数'}排序")
    ...
    def _run_cluster_analysis(self, call: CallbackQuery, cache_key: str, result: dict, token_address: str):
        """在后台运行集群分析"""
        try:
            # 执行集群分析
            ...
            markup = InlineKeyboardMarkup()
            markup.add(InlineKeyboardButton("⬅️ 返回代币排行", callback_data=f"ca1_sort_count_{cache_key}"))
    ...
    def handle_token_detail(self, call: CallbackQuery) -> None:
        ...
        try:
            ...
            if len(parts) >= 3:
                ...
                print(f"详情回调: cache_key={cache_key}, token_index={token_index}, sort={current_sort}")
    ...
    def _handle_token_detail_callback(self, call: CallbackQuery, cache_key: str, token_index: str, current_sort: str):
        """处理代币详情查看逻辑"""
        try:
            # 从缓存中获取分析结果
            if cache_key not in self.analysis_cache:
            ...
            print(f"代币详情回调错误: cache_key={cache_key}, token_index={token_index}, error={str(e)}")
    ...
    def _show_expired_data_option(self, call: CallbackQuery, token_address: str):
        """显示过期数据选项"""
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton("🔄 重新分析", callback_data=f"ca1_reanalyze_{token_address}"))
        self.bot.edit_message_text(
            f"❌ 数据已过期（超过24小时）\n代币: <code>{token_address}</code>\n\n请点击下方按钮重新分析，或重新运行 /ca1 命令",
            call.message.chat.id,
            call.message.message_id,
